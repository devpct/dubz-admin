export default function InventoryModal({isInventoryModalOpen, toggleInventoryModal, consultantName, withdrawAmount, setWithdrawAmount }) {
  return (
    <>
    {isInventoryModalOpen && (
        <div className="w-full h-screen fixed top-0 left-0 z-50 flex items-center justify-center">
        <div className="blur-background" onClick={toggleInventoryModal}></div>
        ...
        <p className='text-[15px]'>ویرایش موجودی {consultantName}</p>
        <svg onClick={toggleInventoryModal} className='cursor-pointer' width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className='mt-5 px-5'>
            ...
            />
            <button onClick={toggleInventoryModal} className='mt-6 w-full h-[48px] bg-[#26397F] text-[14px] text-white rounded-[8px]'>ثبت تغییرات</button>
        </div>
            </div>
        </div>
        </div>
