export default function Card({ name, image, phoneNumber, code, region, stock, fileCount, toggleReportModal, toggleProfileDetailsModal}) {
    return (
        <>
        <div className=' w-full bg-white border border-[#E0DEF7] rounded-[12px]'>
            ...
            <div className='bg-[#cb90441f] px-2 w-[206px] pt-5 pb-2 rounded-[8px] text-center'>
                ...
                <p className='text-[#1a376b] text-[13px]'>{phoneNumber}</p>
            </div>
            ...
            <div className='lg:flex hidden mt-5 gap-3 justify-between items-center'>
                ...
                <button className='w-[117px] h-[40px] text-[#26397F] text-[14px] border border-[#26397F] rounded-[8px] transition-all' onClick={()=>toggleProfileDetailsModal(name, image, phoneNumber, code, region, stock, fileCount)}>جزییات بیشتر</button>
                <button className='w-[104px] h-[40px] text-white text-[14px] border bg-[#26397F] rounded-[8px] transition-all' onClick={()=>toggleReportModal(name)}>مسدودسازی</button>
            </div>
            </div>
            </div>
